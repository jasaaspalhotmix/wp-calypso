 ...
 */
import { get, isEqual, mapValues, omit, omitBy, PropertyPath, reduce } from 'lodash';
...
export type SerializableAction = { type: typeof DESERIALIZE } | { type: typeof SERIALIZE };
 ...
 */
const keyedReducer = < TState, TAction extends { type: string } >(
	keyPath: PropertyPath,
	reducer: Reducer<
		TState | undefined,
		TAction | { type: string } | { type: '@@calypso/INIT' } | SerializableAction
	>
): Reducer< { [ key: string ]: TState }, TAction | SerializableAction > => {
	...
	return (
		...
		action: TAction | SerializableAction
	): { [ key: string ]: TState } => {
		if ( action.type === SERIALIZE ) {
			...
			return ( serialized as unknown ) as { [ key: string ]: TState };
		}
